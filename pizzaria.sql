...
-- selecionar as pizzas que não tem valor cadastratdo valor = null
...
-- selecionar todas as pizzas com valores a R$15 e R$20,00

select * from pizza where valor < 15 or valor = 20;
select * from pizza where valor in (15,20);

-- selecionar todas as pizzas com valores diferentes de R$15 e R$20,00

select * from pizza where valor < 15 end valor != 20;
...
-- desc esc -> opções de ordenação
select * from pizza order by valor desc, nome;

-- selecionar  as 3 mais caras 
select * from pizza order by valor desc limit 4;

-/* Funções de agregação 
*AVG(coluna) Média dos calores da coluna 
*count (coluna) Conta o número de linha
*Max(coluna) Maior valor da coluna
**Min(coluna) Menor valor da coluna
*Sum(coluna) Soma dos valores das colunas
*/
...
select c.cliente_id, c.nome, cont(p.pedido_id) as qtde_pedido
from pedido p
